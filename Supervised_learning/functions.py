...
def predict_t(samples):
    return  np.mean(norm.cdf(samples), axis=0)# TODO: Return p(t=1|samples)


###--- Density functions ---###

def log_prior(u, K_inverse):
    return  -0.5 * np.transpose(u) @ (K_inverse @ u) - len(u)/2 * np.log(2*np.pi) + 0.5 * np.linalg.slogdet(K_inverse)[1] # TODO: Return log p(u)


def log_continuous_likelihood(u, v, G):
    return  - 0.5 * np.linalg.norm((v - G @ u))**2 - (len(v) / 2) * np.log(2 * np.pi)# TODO: Return observation likelihood p(v|u)
...
def grw(log_target, u0, data, K, G, n_iters, beta,N):
    ...
    K_inverse = np.transpose(Kc_inverse) @ Kc_inverse # TODO: compute the inverse of K using its Cholesky decomopsition
    ...
    for i in range(n_iters):
        ...
        log_alpha = min(0,lt_new-lt_prev) # TODO: Calculate acceptance probability based on lt_prev, lt_new
        log_u = np.log(np.random.random())

        # Accept/Reject
        accept = True if log_u <= log_alpha else False# TODO: Compare log_alpha and log_u to accept/reject sample (accept should be boolean)
        if accept:
            acc += 1
            X.append(u_new)
            u_prev = u_new
...
def pcn(log_likelihood, u0, data, K, G, n_iters, beta,N):
    if not 0 < beta <= 1:
    ...
    contraction = np.sqrt(1.0 - beta**2)

    X = np.empty((n_iters, N))
    accepted = 0

    ll_prev = log_likelihood(u_prev, data, G)

    for i in range(n_iters):
        xi = L @ np.random.randn(N)
        u_new = contraction * u_prev + beta * xi

        ll_new = log_likelihood(u_new, data, G)
        log_ratio = ll_new - ll_prev
